...
async function onRoomChange(roomId, form) {
  if (!roomId) { dailyRate.value = 0; form.total = 0; return }
  ...
  >
    <template #default="{ form, submit, loading, isEdit }">
      <q-form @submit.prevent="onSubmit(submit, form)" class="q-gutter-md">
        ...
        <q-input v-model="form.check_in"  type="date" label="Check-in"  filled @update:model-value="() => recomputeTotal(form)" />
        <q-input v-model="form.check_out" type="date" label="Check-out" filled @update:model-value="() => recomputeTotal(form)" />
